/*
 * libobsidian-globalmenu.so
 *
 * Architecture & Purpose:
 * When Chromium / Electron exports menu item accelerators to DBusMenu
 * (com.canonical.dbusmenu), it converts punctuation key codes (such as
 * GDK_KEY_comma / VKEY_OEM_COMMA) into the string name "comma" via
 * XKeysymToString() / gdk_keyval_name() -> shortcut: [["Control", "comma"]].
 *
 ...
 * Because Qt only recognizes literal ASCII punctuation characters (',' not the word "comma"),
 * QKeySequence evaluates to Key_unknown (empty string) and KDE Plasma fails to display
 * the shortcut on the right side of the menu item (e.g. missing "Ctrl+," for Settings).
 *
...
// Convert punctuation key names to literal ASCII characters so Qt/KDE Plasma can parse with QKeySequence
